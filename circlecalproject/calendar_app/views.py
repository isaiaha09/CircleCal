...
def org_custom_domain_settings(request, org_slug):
    ...
    cname_host = None
    cname_target = None
    cf_ssl_status = (getattr(org, 'custom_domain_cloudflare_ssl_status', None) or '').strip()
    cf_dcv_records = getattr(org, 'custom_domain_cloudflare_dcv_records', None) or []
    ...
    return render(request, 'calendar_app/org_custom_domain_settings.html', {
        ...
        'cf_ssl_status': cf_ssl_status,
        'cf_dcv_records': cf_dcv_records,
...
def signup(request):
    ...
    # Mobile app / in-app browser: Turnstile can get stuck in embedded environments.
    # Disable Turnstile when cc_app mode is requested on the Signup page (rate limiting still applies).
    try:
        cc_app_param = request.GET.get('cc_app') == '1'
        cc_app_cookie = request.COOKIES.get('cc_app') == '1'
        if cc_app_param or cc_app_cookie:
            turnstile_enabled = False
            turnstile_site_key = ''
    except Exception:
        pass
